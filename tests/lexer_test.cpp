...
namespace
{
    ...
    void expectTokens(std::string_view input, const std::vector<Expected>& expected, size_t expectedErrors = 0)
    {
        ...
        {
            EXPECT_EQ(tokens[i].type, expected[i].type) << "type mismatch at " << i << " (got " << describe(tokens[i].type) << ")";
...
TEST(Lexer, StringLiteralKeepsInnerSpacesAndPunctuation)
{
    expectTokens(
        R"("hello, world 42!")",
        {
        {TokenType::String, "hello, world 42!"}
    });
}
